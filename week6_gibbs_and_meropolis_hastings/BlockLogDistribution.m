...
%   assignment to these variables consistent with A.
  
%   In the case that |V| = 1,this reduces to Gibbs Sampling.
  
%   NOTE that exp(LogBS) is not normalized to
...
% Hastings, you should make this fast.  
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
temp_factor_index=[];
temp_factor_index=unique(G.var2factors{V});

temp_product=struct('var',[],'card',[],'val',[]);
first=1;
for i=1:length(temp_factor_index),
    temp=setdiff( F(temp_factor_index(i)).var,V);
	%% computational cost be cheaper, we have to consider a case that when both of phi(a|bc) and phi(d|ef) have no connection. 
	%% but there is a assumption that variables in V have the same card. which means, we can use log-sapce to achieve the cheap cost.
    %% we just need to do evidence, phi(factor_involves_i_in_V|setdiff(factor_involces_i_in_V, V))
    %%	
	if length(temp)==0,
	evidence=[];
	else
    evidence=[temp, A(temp)];
	end;
	temp_factor=ObserveEvidence(F(temp_factor_index(i)), evidence);
	temp_factor=FactorMarginalization(temp_factor,temp);
	LogBS+=log(temp_factor.val);
end;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
