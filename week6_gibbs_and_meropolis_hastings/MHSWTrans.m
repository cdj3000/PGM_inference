...
for i = 1:size(G.q_list, 1)  % Iterate through *all* edges, not just the ones we selected earlier
	...
	%G.q_list is pruned q_list
    % A(u) and A(v) reprensent edges in selected cluster
...
stnary_ration= LogProbOfJointAssignment(F, A_prop)-LogProbOfJointAssignment(F, A);
r_x_x_prime=LogR(new_value);

logR_prime=BlockLogDistribution(selected_vars , G, F, A_prop);

r_x_prime_x=logR_prime(old_value);
r_ration=r_x_prime_x  -  r_x_x_prime;
p_acceptance=min(1, exp(stnary_ration + log_QT_ratio + r_ration));
