...
test_list=[];
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if isMax==1,
    for z=1:length(P.cliqueList),
	P.cliqueList(z).val=log(P.cliqueList(z).val);
	P.cliqueList(z).val
	end;
    ...
    while sum(GetNextCliques(P, MESSAGES))!=0,
		...
		test_list=[test_list [indexa indexb]];
		...
		%%% i is not a end node, and ready to marginalize
			...
			temp_product= FactorSum( temp_massage, temp_product );
	    ...
	    MESSAGES(indexa,indexb)= FactorMaxMarginalization(temp_product,  variables_sumed_out );
...
%%temp_list is index for ith messages_list has value
	...
	for k=1:length(temp_list_z),
	    ...
	    temp_product_10= FactorSum( temp_massage_2 , temp_product_10);
	end;
